...
function cocosMake.newSprite(source, x, y, params)
    ...
    if sprite then
        ...
        error(string.format("cocosMake.newSprite() - create sprite failure, source \"%s\"", tostring(source)), 0)
    end
...
--ascii美术字
function cocosMake.newLabelAtlas(txt,pic,w,h,startat)
    return cc.LabelAtlas:_create(txt,pic,w,h,startat)
end
...
function cocosMake.removeAllCacheTextures()
    textureCache:removeAllTextures()
end
function cocosMake.removeAllCacheFramesSprite()
