...
//    FILE *file;
    ...
    unsigned char *p_data = CCFileUtils::sharedFileUtils()->getFileData(filename, "rb", &file_len);
    CCLOG("file_len:%d",file_len);
    char output1[32];
//    if ((file = fopen (filename, "rb")) == NULL)
//    { CCLOG("%s can't be openedn", filename);
//        return 0;
//    }
//    else {
        MD5Init (&context);
...
//        }
        while (offset < file_len) {
            int len = file_len - offset > 1024 ? 1024 : file_len - offset;
            memcpy(buffer, p_data + offset, len);
            offset += len;
